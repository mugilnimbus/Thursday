...
what_it_does: Provides the always-active operating manual for Thursday tool use, skill selection, progressive skill loading, unified tool envelopes, tool result reading, and multi-tool workflows.
when_to_use: Every task that can benefit from tools or skills.
description: What it does: Provides the always-active operating manual for Thursday tool use, skill selection, progressive skill loading, unified tool envelopes, tool result reading, and multi-tool workflows. When to use: Every task that can benefit from tools or skills.
---

# Skill: tool_operations

## Purpose
Use this skill for every task that can benefit from tools. Tools are how you inspect, fetch, edit, run, verify, and complete work. A tool result is evidence; it is not automatically the final answer.

This skill is always active. Other skills are loaded only when you decide they would help.
...
- `capture_webpage` produces visual evidence from a URL, search query, Docker workspace HTML path, or Windows local HTML path. It does not edit files and it is not a source/file-content reader.
