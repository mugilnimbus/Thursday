...
function renderSession(session) {
  ...
  if (messageSignature !== state.lastRenderedMessageSignature && !chatHasSelection) {
    chat.innerHTML = "";
...
async function checkStatus() {
  ...
  try {
    const data = await api(`/api/status?endpoint=${endpoint}`);
    status.className = `pill ${data.ok ? "ok" : "error"}`;
    status.textContent = data.ok ? "LM Studio online" : "LM Studio offline";
  } catch {
...
function renderWorkspace(workspace) {
  ...
  $("resetWorkspaceBtn").disabled = state.workspaceResetting;
}

async function resetWorkspace() {
  if (state.workspaceResetting) return;
  ...
  $("workspaceResetStatus").textContent = "Resetting Docker workspace...";
  try {
...
function renderToolControls() {
  ...
  state.allTools.forEach((tool) => {
    ...
    item.className = `tool-toggle ${enabled.has(name) ? "enabled" : ""}`;
    item.innerHTML = `
      <input type="checkbox" ${enabled.has(name) ? "checked" : ""} />
      <span>
...
function bindUi() {
  ...
  ["endpointInput", "modelInput", "maxTokensInput", "contextInput", "maxStepsInput"].forEach((id) => {
    $(id).addEventListener("change", () => {
      schedulePreferencesSave();
      if (id === "endpointInput") checkStatus();
    });
  });
