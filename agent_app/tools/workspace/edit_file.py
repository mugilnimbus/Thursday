...
TOOL_DEFINITION = {
    ...
    "function": {
        "name": TOOL_NAME,
        "description": (
            "Modify an existing UTF-8 file in the Docker workspace by replacing exact text and returning a unified "
            "diff. Use after read_file when you know the exact old_text. Use expected_replacements=1 for one "
            "targeted edit or 0 to replace every occurrence. Path must be relative to /workspace, not a Windows "
            "host path. For C:\\ host paths, use run_command with PowerShell instead."
        ),
        "parameters": {
            "type": "object",
            "properties": {
