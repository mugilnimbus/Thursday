...
from ..logging import fetch_raw_lmstudio_logs, fetch_recent_logs
from ..runtime import AppState
from ..storage import resolve_served_image
from .http_io import json_response, read_json, read_multipart_image
    ...
    class Handler(BaseHTTPRequestHandler):
        ...
        def do_GET(self) -> None:
            ...
            if path == "/api/images":
                ...
                try:
                    target, content_type = resolve_served_image(config, params.get("path", [""])[0])
            ...
            if path == "/api/status":
                ...
                model = params.get("model", [config.lmstudio_model])[0]
                json_response(self, 200, state.lmstudio_status(endpoint, model))
                return
            if path == "/api/tools":
                json_response(self, 200, state.tools_public())
                return
            if path == "/api/gui-for-llm":
                json_response(self, 200, state.gui_for_llm_public())
                return
        ...
        def do_POST(self) -> None:
            ...
            if parsed.path == "/api/images":
                try:
                    data, filename, mime_type = read_multipart_image(self)
                    json_response(self, 201, state.save_uploaded_image(data, filename, mime_type))
                except Exception as exc:
            ...
            if parsed.path == "/api/preferences":
                ...
                return
            if parsed.path == "/api/gui-for-llm/region":
                payload = read_json(self)
                result = state.update_gui_for_llm_region(payload)
                json_response(self, 200 if result.get("ok") else 400, result)
                return
            if parsed.path == "/api/gui-for-llm/screenshot":
                result = state.capture_gui_for_llm_region()
                json_response(self, 200 if result.get("ok") else 400, result)
                return
